...
async function getGeminiApiKey() {
  ...
  // 1. Check process.env first if available
  if (process.env.GEMINI_API_KEY && process.env.GEMINI_API_KEY !== 'MY_GEMINI_API_KEY') {
    cachedSecretApiKey = process.env.GEMINI_API_KEY;
    return cachedSecretApiKey;
  }

  // 2. Attempt fetching from Secret Manager if project environment is configured
  try {
    ...
    console.warn('[SecretManager] Unable to fetch secret from GCP Secret Manager, falling back to process.env:', err.message);
  }

  // Fallback to process.env or empty
